...
아직 구현을 시작하지 않았다. 현재 산출물은 코드베이스 조사에 기반한 구현 계획이다. 구현 완료 후 실제 변경 파일, 실행한 테스트 명령, 통과 또는 실패 결과, 계획과 달라진 점을 이 섹션에 기록한다.

## 맥락과 방향 안내

저장소 루트는 `/Users/sehako/workspace/stock-report`이다. 백엔드 애플리케이션은 `app/backend` 아래에 있고 Kotlin, Spring Boot, Gradle을 사용한다. 실행 진입점은 `app/backend/src/main/kotlin/com/stockreport/StockReportBackendApplication.kt`이다.
...
작업 디렉터리는 저장소 루트 `/Users/sehako/workspace/stock-report`이다. 백엔드 명령은 `app/backend`에서 실행한다.
