...
- [x] `GET /api/market-indexes/{indexCode}/prices?period=1M|3M|1Y` 엔드포인트를 구현한다.
- [x] 내부 지수 코드 `KOSPI`, `KOSDAQ`만 지원한다.
- [x] `period` 값에 따라 해당 지수의 저장된 최신 거래일을 기준으로 조회 시작일을 계산한다.
- [x] 거래일 오름차순으로 일봉 데이터를 반환한다.
- [x] 존재하지 않는 지수 코드는 404로 처리한다.
- [x] 지원하지 않는 기간 값은 400으로 처리한다.
- [x] `period`가 누락된 경우 400으로 처리한다.
- [x] 데이터가 없는 경우 `startDate`, `endDate`가 `null`이고 `items`가 빈 목록인 200 응답을 반환한다.
...
- [x] `KOSPI`와 `period=3M` 요청 시 지정 기간의 지수 일봉 목록이 거래일 오름차순으로 반환되는지 테스트한다.

## 검증 결과

- `app/backend`에서 `GRADLE_USER_HOME=$PWD/.gradle-local ./gradlew test -q`를 실행해 전체 백엔드 테스트 통과를 확인했다.
- `MarketIndexServiceTest`에서 저장된 최신 거래일 기준 기간 계산, 응답 숫자 정규화, 빈 데이터 응답, 잘못된 지수와 기간 예외를 검증했다.
- `MarketIndexControllerTest`에서 정상 응답 JSON, 빈 목록 응답, 404, 400 상태 코드를 검증했다.
- `MarketIndexPriceRepositoryImplTest`에서 최신 거래일 조회, 기간 범위 조회, 지수 필터, 거래일 오름차순 정렬, OHLCV와 nullable 등락률 매핑을 검증했다.
