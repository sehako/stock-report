...
from jobs.stock_daily_price.domain.model import StockDailyPrice, StockDailyPriceCollectionResult


class StockDailyPriceNormalizationError(ValueError):
    pass


def normalize_stock_daily_prices(stock_id: int, rows: Any) -> StockDailyPriceCollectionResult:
    records = _to_indexed_records(rows)
    prices_by_date: dict[date, StockDailyPrice] = {}
    excluded_count = 0
    duplicate_count = 0
    ...
    for trade_date_value, row in records:
        ...
        if trade_date in prices_by_date:
            duplicate_count += 1
        prices_by_date[trade_date] = StockDailyPrice(
            stock_id=stock_id,
    ...
    if not prices:
        raise StockDailyPriceNormalizationError("저장 가능한 일봉 row가 없습니다.")
    return StockDailyPriceCollectionResult(
        prices=prices,
